...
def test_norm_full_keeps_subtitle():
    ...
    assert catalog_mod._norm_full("Wings of Fire: The Lost Heir") == "wings of fire the lost heir"
    assert catalog_mod._norm_full("Wings of Fire: The Lost Heir") != catalog_mod._norm_full("Wings of Fire: The Dragonet Prophecy")


def test_search_dedup_key_distinguishes_series_volumes():
    k7 = catalog_mod._search_dedup_key("Wings of Fire: The Dark Secret", "Tui T. Sutherland")
    k8 = catalog_mod._search_dedup_key("Wings of Fire: The Brightest Night", "Tui T. Sutherland")
    assert k7 != k8


def test_match_score_bands():
    q = "the fault in our stars"
    assert catalog_mod._match_score(q, _c("The Fault in Our Stars")) == 100
    assert catalog_mod._match_score("the fault", _c("The Fault in Our Stars")) == 80   # startswith
    assert catalog_mod._match_score("fault stars", _c("The Fault in Our Stars")) == 60  # all tokens
    assert catalog_mod._match_score("our stars", _c("The Fault in Our Stars")) == 60  # all tokens
    assert catalog_mod._match_score("geology", _c("Faults and Earthquakes", author="A. Geologist")) == 0
    assert catalog_mod._match_score("green", _c("The Fault in Our Stars", author="John Green")) == 20  # author only
...
def test_rank_key_orders_title_match_over_keyword():
    ...
    ranked = sorted([keyword_hit, title_hit], key=lambda c: catalog_mod._rank_key(q, c), reverse=True)
...
def test_volume_number_parsing():
    assert catalog_mod._volume_number("Wings of Fire #7") == 7
    assert catalog_mod._volume_number("Wings of Fire, Book 8") == 8
    assert catalog_mod._volume_number("Wings of Fire: Volume 3") == 3
    assert catalog_mod._volume_number("The Fault in Our Stars") is None
