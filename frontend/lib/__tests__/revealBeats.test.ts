...
function trait(over: Partial<Trait>): Trait {
  return {
    id: 1, claim: 'c', reveal_line: 'You do a thing.', polarity: 'reward',
    exhibits: [], contrasts: [], inference_confidence: 0.8, status: 'proposed',
    user_weight: 1, user_note: null, created_at: '',
    ...over,
  };
}
...
const archetype: ArchetypeOut = {
  code: 'ICDH', name: 'The Devoted Fan', tagline: 'I live in this world now.',
  hook: 'reread the whole series to get ready for the new one',
  lens: axis(-0.6, 'I'), engine: axis(0.7, 'C'), range: axis(0.5, 'D'),
  resonance: axis(-0.4, 'H'), derived_at: '', is_stale: false,
};

const highlights: ProfileHighlights = {
  thin: false, n_authors: 40,
  top_genres: [{ subject: 'Fantasy', share: 0.5 }, { subject: 'Sci-Fi', share: 0.3 }],
  top_authors: ['Le Guin', 'Mitchell', 'Chekhov'],
  format_mix: { novel: 3, novella: 1, collection: 0, series: 6, dominant: 'series', low_confidence: false },
  era_split: { pre_2000: 10, post_2000: 30 },
};

const stats: Stats = {
  total: 120, rated: 100, unrated: 20, shelves: {}, mean_rating: 4.3,
...
describe('buildBeats', () => {
  ...
  it('caps reward-trait beats at 2 and drops axes for a thin library', () => {
    ...
    const beats = buildBeats({
      stats, traits: many, archetype,
      highlights: { ...highlights, thin: true }, books,
    });
  ...
  it('groups aversions into a single beat', () => {
    const beats = buildBeats({
      stats,
      traits: [trait({ id: 1, polarity: 'reward' }), trait({ id: 2, polarity: 'aversion' }), trait({ id: 3, polarity: 'aversion' })],
      archetype, highlights, books,
    });
  ...
  it('marks low-confidence reward traits', () => {
    const beats = buildBeats({
      stats, traits: [trait({ id: 1, inference_confidence: 0.2 })],
      archetype, highlights, books,
    });
