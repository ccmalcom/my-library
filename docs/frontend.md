...
**No longer a pure HTTP client.** Through waves 0–5a the frontend grew its own backend: Next.js
route handlers under `frontend/app/api/**`, backed by drizzle over the same Supabase Postgres
(`frontend/lib/server/**`). It talks to the database directly, verifies Supabase JWTs itself, and
owns rate limiting and encryption. Alembic is still the single source of truth for **schema**, so
the frontend runs no migrations — that much of the old rule survives.

Which backend serves a given call is decided per route by `lib/backend.ts`: `NODE_DEFAULT_ROUTES`
lists what `auto` mode sends to Node, `NODE_ONLY_PREFIXES` what exists on Node alone, and the
`python` / `node` overrides (admin System tab) force one side. As of wave 5b that table covers
every frontend-facing Python route, and Python/Railway is being retired — see `docs/hosting.md`.

## Auth (Supabase, auth-only)

Supabase is used purely to get a session — never to query tables from the browser (the backend owns
data, whether that is FastAPI or the Node route handlers). `utils/supabase/client.ts` is the singleton browser client; `authEnabled` is false when the `NEXT_PUBLIC_SUPABASE_*` env vars are absent, so **local dev runs unauthenticated exactly as before**. `lib/api.ts` `authHeaders()` attaches the session's `access_token` as `Authorization: Bearer` on every request (the backend verifies it via JWKS). `middleware.ts` refreshes the session and redirects unauthenticated users to `/login` (no-op in local mode); `app/login` is the email+password sign-in (invite-only, no sign-up form). Supabase publishable key env var is `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`.
...
**Invite/recovery links must land on `/auth/callback`** (the only page that consumes the hash tokens and prompts for a password). For that, Supabase must be told to redirect there — which requires **both** the backend env `FRONTEND_URL` — on Railway for the Python path, and in Vercel for the Node path, where `invites.ts::createInvite` uses it the same way — (so the invite call sends `redirect_to=…/auth/callback`) **and** that exact URL in the Supabase dashboard's Auth → URL Configuration → **Redirect URLs** allowlist. If either is missing, GoTrue silently falls back to the project Site URL (bare app root), which `middleware.ts` bounces to `/login` with the tokens stranded in the URL hash. As a safety net, `app/login` forwards any invite/recovery hash to `/auth/callback` via `lib/authRedirect.ts` (`inviteCallbackRedirect`) so onboarding still completes even when that config is wrong.

## Key files

- `lib/backend.ts` — the per-route backend switcher. `NODE_DEFAULT_ROUTES` (prefix + optional `methods` + optional `exact`) is what `auto` mode serves from Node; `NODE_ONLY_PREFIXES` is Node-only. Several entries' `exact` flags are load-bearing — `/recommend` vs `/recommendations`, `/import` vs `/import/preview`, `/enrich` vs `/enrich/start` — and the comments say which and why. Read them before adding a rule.
- `lib/api.ts` — single typed fetch client. All calls go through it; the base is chosen per call by `lib/backend.ts`, resolving to `/api` for Node routes or `NEXT_PUBLIC_API_URL` (default `http://127.0.0.1:8000`) for Python ones. Types here mirror the Pydantic schemas. `PROFILE_STATUS_KEY` is the shared SWR key for `/profile/status` so a mutation anywhere can revalidate the re-profile banner. `DIRECTIVE_KEY` is the shared SWR key for the custom-instructions record (`Directive` / `DirectiveConstraints` / `DirectiveDraft` types; `getDirective` / `putDirective` / `deleteDirective` / `draftDirective` calls hitting `GET/PUT/DELETE /directive` + `POST /directive/draft`).
