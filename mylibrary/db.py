"""Database: SQLAlchemy 2.0 models, migrations, and session management.
...
class TasteSignal(Base):
    """Durable append-only steering signals: more/less like a specific book or rec.

    Never dropped by clear_library or clear_profile — these are irreplaceable user
    preferences that survive full library resets. direction: more|less.
    target_kind: book|rec. target_book_id references the user's own Book row (for
    book-kind signals). snapshot stores a rec's title/author/subjects for rec-kind
    signals (the rec row may be pruned later).
    """

    __tablename__ = "taste_signal"
    ...
    direction: Mapped[str] = mapped_column(String, nullable=False)   # more | less
    target_kind: Mapped[str] = mapped_column(String, nullable=False)  # book | rec
...
def init_db() -> None:
    ...
    #   - Pre-exhibits/contrasts schema → still safe to drop+recreate (no verdicts).
    #   - Modern schema (exhibits+contrasts present) → add missing columns in place
    #     so existing verdicts survive.
    if "taste_traits" in insp.get_table_names():
        ...
        if not {"exhibits", "contrasts"} <= cols:
            ...
            # Add any missing columns in place so existing verdicts survive.
            # Hardcoded list (mirrors books migration style) — intentionally does not
            # read from TasteTrait.__table__.columns so the migration works even when a
            # stale .pyc makes the ORM model lag behind the source file.
            new_trait_cols = {
            ...
            if missing_trait_cols:
                with engine.begin() as conn:
                    for col_name, col_type in missing_trait_cols.items():
                        conn.execute(
                            sa_text(
                                f"ALTER TABLE taste_traits ADD COLUMN {col_name} {col_type}"
                            )
                        )
    ...
    if "recommendations" in insp.get_table_names():
        ...
        if not {"run_id", "status"} <= cols:
            ...
            # Hardcoded list (mirrors taste_traits migration style) — intentionally does not
            # read from Recommendation.__table__.columns so the migration works even when a
            # stale .pyc makes the ORM model lag behind the source file.
            new_rec_cols = {
                "reject_reasons": "JSON",
            }
            missing_rec_cols = {k: v for k, v in new_rec_cols.items() if k not in cols}
            if missing_rec_cols:
                with engine.begin() as conn:
                    for col_name, col_type in missing_rec_cols.items():
                        conn.execute(
                            sa_text(
