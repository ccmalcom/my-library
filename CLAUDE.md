...
## Web distribution (done - beta testing)
...
- **Alembic** owns the hosted schema: `alembic.ini` + `alembic/env.py` (pulls `settings.db_url`)
  - `alembic/versions/0001_initial_multitenant_schema.py` (baseline from `Base.metadata`).
    ...
    **Migration baseline gotcha (fixed):** `0001_initial` builds the schema via
    `Base.metadata.create_all()` from the _live_ models — so as the models gained `display_name`
...
- **No non-ASCII characters inside JS string literals in `.tsx` files.** Turbopack's
  ...
  quotes (`"` `"`), ellipses (`…`), etc. are fine in JSX _text nodes_ (between tags)
...
- **`recommend` requires a clean, up-to-date profile** — it raises `RuntimeError` (→ HTTP 400) if `last_profiled_at` is `None` (no profile ever built) or if any rated/reviewed book
...
- **`exclude_from_profile`** (`Book` column, bool, default False): tracks a book without
  including it in taste profiling or archetype derivation. Toggling it goes through
  `set_book_feedback` (bumps `feedback_updated_at`, dirtying the profile). `build_tiers`
  and `books_changed_since` in `profile.py` both filter `exclude_from_profile == False`.
  Alembic migration: `0006_add_exclude_from_profile` (idempotent). Frontend: toggle in
  `BookEditModal`; "excluded" badge on library rows.
...
- **First-run gating (`LibraryGate`) / SWR cache**: a user with no library no longer gets
  _redirected_ to `/setup`. Instead `LibraryGate` (mounted in `app/(main)/layout.tsx`) renders
  ...
  A bare call only _revalidates_, and SWR won't refetch a key no mounted component subscribes to,
